public class LeasingCost {
    ...
    public static Vehicle createVehicle(String description) {
        ...
        int indexOfName = description.indexOf("name:") + "name:".length();
        String nameDescription = description.substring(indexOfName);
        int semiColon = nameDescription.indexOf(";");
        nameDescription = nameDescription.substring(0,semiColon);
        System.out.println(nameDescription);

        int indexOfDownPay = description.indexOf("due:") + "due:".length();
        String downPayDescription = description.substring(indexOfDownPay);
        semiColon = downPayDescription.indexOf(";");
        downPayDescription = downPayDescription.substring(0,semiColon);
        System.out.println(downPayDescription);
        double downPay = Double.parseDouble(downPayDescription);

        int indexOfTime = description.indexOf("length:") + "length:".length();
        String timeDescription = description.substring(indexOfTime);
        semiColon = timeDescription.indexOf(";");
        timeDescription = timeDescription.substring(0,semiColon);
        int timeLease = Integer.parseInt(timeDescription);

        int indexOfMonths = description.indexOf("monthly:") + "monthly:".length();
        String monthsDescription = description.substring(indexOfMonths);
        semiColon = monthsDescription.indexOf(";");
        monthsDescription = monthsDescription.substring(0,semiColon);
        double monthsCost = Integer.parseInt(monthsDescription);

        int indexOfLimit = description.indexOf("allowance:") + "allowance:".length();
        String limitDescription = description.substring(indexOfLimit);
        semiColon = limitDescription.indexOf(";");
        limitDescription = limitDescription.substring(0,semiColon);
        int mileageLimit = Integer.parseInt(limitDescription);

        Lease lease = new Lease(downPay, timeLease, monthsCost, mileageLimit);

        int indexOfFuel = description.indexOf("type:") + "type:".length();
        String fuelDescription = description.substring(indexOfFuel);
        semiColon = fuelDescription.indexOf(";");
        fuelDescription = fuelDescription.substring(0,semiColon);

        int indexOfGas = description.indexOf("mile/unit:") + "mile/unit:".length();
        String gasDescription = description.substring(indexOfGas);
        semiColon = gasDescription.indexOf(";");
        gasDescription = gasDescription.substring(0,semiColon);
        double usage = Double.parseDouble(gasDescription);

        Fuel fuel;
        if(fuelDescription.equals("gas")){
            fuel = new Fuel(usage);
        } else{
            int indexOfCharger = description.indexOf("charger:") + "charger:".length();
            String chargerDescription = description.substring(indexOfCharger);
            semiColon = chargerDescription.indexOf(";");
            chargerDescription = chargerDescription.substring(0,semiColon);
            double charger = Double.parseDouble(chargerDescription);
            fuel = new Fuel(usage,charger);
        }


        return new Vehicle (nameDescription, fuel, lease);
    ...
    public static void computeCO2EmissionsAndCost(Vehicle[] vehicles, double gasPrice, double electricityPrice) {
        ...
        for(int j=0; j<vehicles.length; j++){
            if( Fuel.GAS == vehicles[j].getFuel().getType()){
                vehicles[j].setCO2Emission(computeCO2(vehicles[j].getLease().getLeaseLength(), vehicles[j].getFuel().getUsage(), vehicles[j].getLease().getMileageAllowance(), Fuel.CO2EMITTED_GASCOMBUSTION));
            } else {
                vehicles[j].setCO2Emission(computeCO2(vehicles[j].getLease().getLeaseLength(), vehicles[j].getFuel().getUsage(), vehicles[j].getLease().getMileageAllowance(), Fuel.CO2EMITTED_ELECTRICITYCOMBUSTION));
            }

            if(vehicles[j].getFuel().getType() == Fuel.GAS){
                vehicles[j].setFuelCost(computeFuelCost(vehicles[j].getLease().getLeaseLength(), vehicles[j].getFuel().getUsage(), vehicles[j].getLease().getMileageAllowance(), gasPrice));
            } else{
                vehicles[j].setFuelCost(computeFuelCost(vehicles[j].getLease().getLeaseLength(), vehicles[j].getFuel().getUsage(), vehicles[j].getLease().getMileageAllowance(), electricityPrice));
            }

            if(vehicles[j].getFuel().getType()==Fuel.GAS){
                vehicles[j].setTotalCost(vehicles[j].getFuelCost() + (computeLeaseCost(vehicles[j].getLease().getDueAtSigning(), vehicles[j].getLease().getLeaseLength(), vehicles[j].getLease().getMonthlyCost())));
            } else{
                vehicles[j].setTotalCost(vehicles[j].getFuelCost() + (computeLeaseCost(vehicles[j].getLease().getDueAtSigning(), vehicles[j].getLease().getLeaseLength(), vehicles[j].getLease().getMonthlyCost())) + vehicles[j].getFuel().getCharger());
            }
        }
    }
