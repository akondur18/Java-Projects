...
public class WeatherGenerator {
    ...
    public static void populateLocationProbabilities( double[] drywetProbability, double[] wetwetProbability, 
        ...
        int wetwetIndex = 0;
        for (int i = 0; i<drywet.length; i++) {
            if (drywet[i][0]==lon && drywet[i][1]==lat) {
                drywetIndex = i;
                break;
            }
        }
        for (int i = 0, j = 2; i<12; i++, j++) {
            drywetProbability[i] = drywet[drywetIndex][j];
        }
        for (int i = 0; i<wetwet.length; i++) {
            if (wetwet[i][0]==lon && wetwet[i][1]==lat) {
                wetwetIndex = i;
                break;
            }
        }
        for (int i = 0, j=2; i<12; i++, j++) {
            wetwetProbability[i] = wetwet[wetwetIndex][j];
        }
    }
    ...
    public static int[] oneMonthForecast(int numberOfLocations, int month, double longitude, double latitude ){
        ...
        double wwp[] = new double[12];
        double dwp[] = new double[12];

        populateLocationProbabilities(dwp, wwp, longitude, latitude, dw, ww);
        return forecastGenerator(dwp[month], wwp[month], numberOfDaysInMonth[month]);
    }
    ...
    public static int lengthOfLongestSpell (int[] forecast, int mode) {
        ...
        int count = 0;
        int idx = 0;
        int arr[] = new int[forecast.length];
        for(int i = 0; i<forecast.length; i++) {
            if(i == forecast.length-1) {
                if(forecast[forecast.length-1]==mode) {
                    count++;
                    arr[idx] = count;
                }
            }
            else if (forecast[i]==mode && forecast[i+1] != mode) {
                count++;
                arr[idx] = count;
                idx++;
                count = 0;
            }
            else if (forecast[i] == mode) {
                count++;
            }
        }
        int longest = 0;
    ...
    public static int bestWeekToTravel(int[] forecast){
        ...
        // COMPLETE THIS METHOD
        int count = 0;
        int best[] = new int[forecast.length-6];
